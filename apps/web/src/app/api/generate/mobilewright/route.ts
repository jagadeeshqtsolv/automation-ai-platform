...
export async function POST(req: Request) {
  ...
  if (platform === "web") {
    return NextResponse.json(
      { error: "This project uses Playwright (web). Use web codegen from Test plans." },
      { status: 400 },
    );
  }
  ...
  try {
    const { bundle, model } = await generateMobilewrightPomBundle({
    ...
    const seen = new Set<string>(["support/locate.ts", "support/actions.ts"]);
    ...
    if (!seen.has(TEST_FIXTURES_MODULE_PATH)) {
      ...
      diskFiles.push({
        ...
        content: generateTestFixturesSource(fixturePageRows),
      });
    }
