        ...
        <div className="mx-auto max-w-6xl px-6 py-12 md:py-16">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 text-lg leading-relaxed text-slate-500">
              {BRAND_TAGLINE}. This guide covers both <strong className="font-medium text-slate-600">web</strong> projects
              (Playwright) and <strong className="font-medium text-slate-600">mobile</strong> projects (Mobilewright) —
          ...
          <div className="mt-12 grid gap-10 lg:grid-cols-[220px_1fr] lg:gap-14">
            ...
            <article className="min-w-0 space-y-14">
              <DocSection id="overview" title="Overview">
                ...
                <p className="mt-3">
                  When you <strong className="text-slate-700">create a project</strong>, choose the platform:
                </p>
                <ul className="mt-2 list-disc space-y-1 pl-5 text-slate-600">
                  <li>
                    <strong className="text-slate-700">Web (Playwright)</strong> — browser automation, DOM recorder,
                    page objects with CSS/role locators, iframe and shadow DOM support.
                  </li>
                  <li>
                    <strong className="text-slate-700">Mobile (Mobilewright)</strong> — iOS/Android simulators and
              ...
              <DocSection id="before-you-begin" title="Before you begin">
                ...
                <h3 className="mt-6 text-base font-semibold text-slate-900">OpenAI API key</h3>
                <p className="mt-2">
                  Test Plans, page objects, and generated specs use each project&apos;s OpenAI key — not a shared
                  platform key. After sign-in, open a project → <strong className="text-slate-700">Setup</strong> → save
                  your API key before generating anything.
                </p>

                <h3 className="mt-6 text-base font-semibold text-slate-900">Local test runs</h3>
                <p className="mt-2">
                  The app writes an isolated framework folder on the machine where the web app runs (see{" "}
                  <a href="#framework" className="text-green-700 hover:underline">
                    Framework on disk
                  </a>
                ...
                <ul className="mt-2 list-disc space-y-2 pl-5 text-slate-600">
                  <li>
                    <strong className="text-slate-700">Web</strong> — Playwright browsers install on first run (
                    <code className="font-mono text-xs text-slate-500">npx playwright install</code> if you run CLI
                    manually). Set a reachable <code className="font-mono text-xs text-slate-500">baseURL</code> in your
                    environment.
                  </li>
                  <li>
                    <strong className="text-slate-700">Mobile</strong> — Boot the iOS simulator or Android emulator that
              ...
              <DocSection id="quick-start" title="Quick start">
                <ol className="space-y-6">
                  <QuickStep n={1} title="Sign in & create a project">
                    Open the dashboard after login. Click <strong className="text-slate-700">New Project</strong>, enter a
                    name, and pick <strong className="text-slate-700">Web (Playwright)</strong> or{" "}
                    <strong className="text-slate-700">Mobile (Mobilewright)</strong>. Platform cannot be changed later.
                  </QuickStep>
                  <QuickStep n={2} title="Setup">
                    In the project workspace, open <strong className="text-slate-700">Setup</strong>: save your OpenAI
                  ...
                  <QuickStep n={3} title="Add a requirement">
                    Under <strong className="text-slate-700">Requirements</strong>, paste acceptance criteria or user
                    stories. Generate a validated test plan with cases and steps.
                  </QuickStep>
                  <QuickStep n={4} title="Page Objects">
                    Open <strong className="text-slate-700">Recorder</strong>: capture the page or device tree, select
                    ...
                    <strong className="text-slate-700">Page Objects</strong> or <strong className="text-slate-700">Framework</strong>.
                  </QuickStep>
                  <QuickStep n={5} title="Generate & run">
                    In <strong className="text-slate-700">Test Plans</strong>, generate Playwright or Mobilewright specs
                    for a suite. Open <strong className="text-slate-700">Test Execution</strong>, select spec files, and
                    click Run. Logs stream in the console; use <strong className="text-slate-700">Test Reports</strong>{" "}
              ...
              <DocSection id="page-objects" title="Page Objects & recorder">
                <h3 className="text-base font-semibold text-slate-900">Web recorder (Playwright)</h3>
                <p className="mt-2">
                  ...
                  <code className="font-mono text-xs text-slate-500">webLocator</code> helpers — including nested{" "}
                  <code className="font-mono text-xs text-slate-500">frame</code> and{" "}
                  <code className="font-mono text-xs text-slate-500">shadowHost</code> chains when you record inside
                  iframes or shadow DOM.
                </p>

                <h3 className="mt-6 text-base font-semibold text-slate-900">Mobile recorder (Mobilewright)</h3>
                <p className="mt-2">
                  ...
                  <code className="font-mono text-xs text-slate-500">locate</code> helper in the mobile framework.
                </p>
              ...
              <DocSection id="tests" title="Generate & run tests">
                <h3 className="text-base font-semibold text-slate-900">Codegen</h3>
                <p className="mt-2">
                ...
                <h3 className="mt-6 text-base font-semibold text-slate-900">Test Execution</h3>
                <ul className="mt-2 list-disc space-y-2 pl-5 text-slate-600">
                  <li>Pick one or more spec files from the list (or Select All).</li>
                  <li>
                    <strong className="text-slate-700">Environment</strong> — leave default to use values from{" "}
                    <code className="font-mono text-xs text-slate-500">playwright.config.ts</code> or{" "}
                    <code className="font-mono text-xs text-slate-500">mobilewright.config.ts</code>, or pick a specific
                    saved environment.
                  </li>
                  <li>
                    Optional <strong className="text-slate-700">grep</strong> filter (e.g.{" "}
                    <code className="font-mono text-xs text-slate-500">@smoke</code>) to run a subset of tests.
                  </li>
                  <li>
                    Click <strong className="text-slate-700">Run</strong> — logs poll until the run passes, fails, or is
                    stopped. Use <strong className="text-slate-700">Rerun all</strong> or{" "}
                    <strong className="text-slate-700">Rerun failures</strong> after a completed run.
                  </li>
                  <li>Only one active run per project at a time.</li>
                </ul>

                <h3 className="mt-6 text-base font-semibold text-slate-900">Heal (mobile & web)</h3>
                <p className="mt-2">
                  After a failed run, you can trigger <strong className="text-slate-700">Heal</strong> to send failure
              ...
              <DocSection id="framework" title="Framework on disk">
                ...
                <ul className="mt-3 list-disc space-y-2 pl-5 font-mono text-xs text-slate-500">
                ...
                <ul className="mt-2 list-disc space-y-2 pl-5 font-mono text-xs text-slate-500">
                ...
                <p className="mt-3">
                  ...
                  <code className="font-mono text-xs text-slate-500">npm test</code> /{" "}
                  <code className="font-mono text-xs text-slate-500">npx playwright test</code> inside that folder
                  locally.
                </p>
              </DocSection>
              ...
              <DocSection id="tips" title="Tips & troubleshooting">
                <ul className="list-disc space-y-3 pl-5 text-slate-600">
                  <li>
                    <strong className="text-slate-700">Generation fails with 503</strong> — Add or verify the OpenAI API
                    key in Setup for this project.
                  </li>
                  <li>
                    <strong className="text-slate-700">No spec files listed</strong> — Generate tests first; specs must
                    exist under <code className="font-mono text-xs">tests/</code>.
                  </li>
                  <li>
                    <strong className="text-slate-700">Run hangs on install</strong> — First run installs npm dependencies
                    in the framework folder; watch the live log for progress.
                  </li>
                  <li>
                    <strong className="text-slate-700">Web: navigation or timeout errors</strong> — Check{" "}
                    <code className="font-mono text-xs">baseURL</code> in the environment and that the app is reachable
                    from the runner machine.
                  </li>
                  <li>
                    <strong className="text-slate-700">Mobile: simulator not found</strong> — Boot the correct iOS
                    simulator or Android emulator and align bundle ID / platform in your environment.
                  </li>
                  <li>
                    <strong className="text-slate-700">Cloud runs</strong> — Configure provider username and access key in
                    Setup; execution writes a generated <code className="font-mono text-xs">.env.execution</code> at run
                    time.
                  </li>
                  <li>
                    <strong className="text-slate-700">Wrong platform tools</strong> — Web projects use Playwright only;
                    mobile projects use Mobilewright. Create a separate project if you need both targets.
                  </li>
                </ul>
              </DocSection>

              <div className="rounded-2xl border border-green-200 bg-gradient-to-br from-accent/10 via-white/90 to-white px-6 py-8 text-center">
...
function DocNav() {
  return (
    <nav className="hidden lg:block" aria-label="On this page">
      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500">On this page</p>
      <ul className="mt-3 space-y-1 border-l border-slate-200">
        {SECTIONS.map((s) => (
          <li key={s.id}>
            <a
              href={`#${s.id}`}
              className="block border-l-2 border-transparent py-1.5 pl-3 text-sm text-slate-500 transition hover:border-green-200 hover:text-green-700"
            >
              {s.label}
            </a>
          </li>
...
function DocSection({
  ...
  return (
    <section id={id} className="scroll-mt-24">
      ...
      <div className="mt-4 space-y-3 text-sm leading-relaxed text-slate-500">{children}</div>
    </section>
  );
}
...
function QuickStep({ n, title, children }: { n: number; title: string; children: ReactNode }) {
  return (
    <li className="flex gap-4">
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-accent/15 font-mono text-sm font-semibold text-green-700">
        {n}
      </span>
      <div>
...
function PlatformRow({
  ...
  return (
    <tr className="border-b border-slate-200 last:border-0">
      <td className="px-4 py-3 font-medium text-slate-700">{topic}</td>
      <td className="px-4 py-3 text-slate-500">{web}</td>
      <td className="px-4 py-3 text-slate-500">{mobile}</td>
    </tr>
  );
}
