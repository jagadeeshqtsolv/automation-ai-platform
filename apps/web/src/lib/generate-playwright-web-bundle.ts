...
function buildPreconditionsRule(authFile?: string): string {
  const loginRule = authFile
    ? `(2) Login/authentication e.g. 'User is logged in', 'authenticated', 'valid credentials' → skip entirely — the browser context is pre-authenticated via storageState (".auth/${authFile}"); no login steps are needed`
    : `(2) Login/authentication e.g. 'User is logged in', 'authenticated', 'valid credentials' → use the loginPage fixture: fill credentials from testData then click login, e.g. \`await loginPage.fillUsername(testData.auth.username); await loginPage.fillPassword(testData.auth.password); await loginPage.clickLoginButton();\``;
...
function buildWebPomRules(authFile?: string, hasTestData?: boolean): string {
  ...
  "Use frame for elements inside iframes; shadowHost for open shadow DOM hosts. New tabs: use clickOpensNewPage(page, locator) when clicking opens a new tab; use waitForNewPage(page) to wait for an already-triggered popup. SALESFORCE lightning-button RULE: `lightning-button` stores the `name` attribute on the outer custom element itself, NOT in its shadow DOM — so use strategy 'css' with value 'lightning-button[name=\"Xyz\"]' (no shadowHost). Only use shadowHost for components where the target attribute/text lives INSIDE the shadow root (e.g. lightning-button-icon, lightning-primitive-input-simple, lightning-base-combobox).",
...
export async function generatePlaywrightWebPomBundle(params: {
  ...
  const authContext = params.authFile
    ? `Auth: storageState pre-configured — ".auth/${params.authFile}" is set in playwright.config.ts. Tests start already authenticated. Do NOT generate login steps.`
    : "";
