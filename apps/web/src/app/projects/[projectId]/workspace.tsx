...
export function ProjectWorkspace({ projectId }: { projectId: string }) {
  ...
  const overwritePageObjectsFromTests = false;
  ...
  const load = useCallback(async () => {
    ...
    setProject(data);
    // Auto-select the first environment as default if none selected yet
    setSelectedEnvId((prev) =>
      prev === "" && data.environments.length > 0 ? data.environments[0]!.id : prev,
    );
  }, [projectId, toast]);
  ...
  return (
    <>
      <div className="flex flex-col gap-6 lg:flex-row lg:items-stretch lg:gap-4">
        ...
        <div className="min-w-0 flex-1 space-y-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm lg:p-5">
          {activeTab === "overview" ? (
          ...
          {activeTab === "setup" ? (
            <ProjectSetupSection
              ...
              onEnvironmentSaved={setSelectedEnvId}
            />
          ) : null}
    ...
    <section className="ui-panel-accent animate-slide-up overflow-hidden">
      <div className="ui-panel-header flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <p className="ui-eyebrow text-green-700">Download framework</p>
      ...
      <div className="ui-panel-body space-y-4">
        <div className="rounded-xl border border-slate-200 bg-slate-100 px-4 py-3">
          ...
          <p className="mt-1 font-mono text-xs text-slate-600">{rootPath ?? hint}</p>
        </div>
...
function MemberReadOnlyNotice({ tab }: { tab: string }) {
  return (
    <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-500/8 px-3 py-2 text-xs text-amber-700/80">
      <svg className="h-3.5 w-3.5 shrink-0 text-amber-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden>
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" />
      </svg>
      <span>
        <span className="font-semibold text-amber-700">{tab} settings are read-only for members.</span>{" "}
...
function ProjectSetupSection(props: {
  ...
  onEnvironmentSaved?: (envId: string) => void;
}) {
  const toast = useToast();
  const { project, projectId, busy, isOwner, onBusy, onReload, onEnvironmentSaved } = props;
  ...
  async function saveEnvironment(e: FormEvent) {
    ...
    try {
      ...
      const savedId = saved.id ?? (isEditing ? editingEnvironmentId ?? "" : "");
      resetEnvironmentForm();
      await onReload();
      if (savedId) onEnvironmentSaved?.(savedId);
      toast.success(isEditing ? "Environment updated — set as default" : "Environment added — set as default");
    } finally {
      onBusy(null);
    }
  }
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
      <header className="border-b border-slate-200 px-6 pt-6 pb-0">
        <div className="mb-4">
        ...
        {showSetupBanner && (
          <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
            <p className="text-sm font-medium text-amber-600">Complete setup before using this project</p>
            <ul className="mt-2 space-y-1 text-xs text-amber-700/70">
              {!aiConfigured && (
                <li>
                  <button
                    type="button"
      ...
      <div className="p-6 space-y-6">
        ...
        {setupTab === "environments" && <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-4">
          ...
          <ul className="space-y-2 text-sm text-slate-600">
            {project.environments.length === 0 ? <li className="text-slate-500">No environments yet.</li> : null}
            {project.environments.map((env) => {
              let pretty = env.configJson;
              try { pretty = JSON.stringify(JSON.parse(env.configJson) as unknown, null, 2); } catch { /* keep raw */ }
              return (
                <li key={env.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                  <div className="flex items-center justify-between gap-2">
                    <p className="font-medium text-slate-900">
                      {env.name}{" "}
                      <span className="text-xs font-normal text-slate-500">({env.slug})</span>
                    </p>
                  ...
                  <pre className="mt-2 max-h-40 overflow-auto rounded-md bg-slate-100 p-2 text-[11px] leading-relaxed text-slate-500 whitespace-pre">{pretty}</pre>
                </li>
              );
            })}
          </ul>

          <form className="space-y-2 border-t border-slate-200 pt-3" onSubmit={saveEnvironment}>
            ...
            <label className="text-xs text-slate-500">
              ...
              <textarea
                ...
                rows={10}
                className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 font-mono text-[11px] text-slate-700"
              />
            </label>
